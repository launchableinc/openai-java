...
import com.launchableinc.openai.ListSearchParameters;
...
public class AssistantTest {
	...
	void createAssistant() {
		AssistantRequest assistantRequest = AssistantRequest.builder()
				.model(TikTokensUtil.ModelEnum.GPT_4_1106_preview.getName()).name("Math Tutor")
	...
	void modifyAssistant() {
		...
		ModifyAssistantRequest modifyRequest = ModifyAssistantRequest.builder().name(modifiedName)
				.build();
	...
	@Order(4)
	void listAssistants() {
		OpenAiResponse<Assistant> assistants = service.listAssistants(
	...
	void listAssistantFiles() {
		List<AssistantFile> files = service.listAssistantFiles(assistantId,
				new ListSearchParameters()).data;
